...
bool threadWorkDone[MAX_THREADS] = {
    0,
};
...
bool allThreadsComplete() {
	for (int i = 0; i < meshSize; i++) {
		if (!threadWorkDone[i]) return false;
	}
...
void *meshThreadFunc(void *arg) {
	...
	for (int phase = 0; phase <= log2(meshSize)+1; phase++) {
		...
		if (!allThreadsComplete()) { // If not all threads are complete, wait
			...
			for (size_t i = 0; i < meshSize; i++) threadWorkDone[i] = false; // Reset thread work
